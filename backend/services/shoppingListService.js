...
const ShoppingListService = {
  async generateFromWeek(userId, startDate, endDate) {
    const result = await db.execute({
      sql: `SELECT i.id AS ingredient_id, i.category, i.name, i.price_per_unit,
    ...
    if (ingredientIds.length) {
      ...
      const storeResult = await db.execute({
        sql: `SELECT isp.ingredient_id, isp.store, isp.price, isp.product_url
              FROM ingredient_store_prices isp
              WHERE isp.ingredient_id IN (${placeholders})`,
        args: ingredientIds,
      });
      for (const sp of storeResult.rows) {
  ...
  async getStorePrices(ingredientId) {
    const result = await db.execute({
      sql: `SELECT isp.store, isp.price, isp.product_url,
